...
{	public function rateImage($user_images_id,$rating){	$this->db->query("insert into user_images_rating(user_images_id,rating)value('".$user_images_id."','".$rating."')");		} 	public function rating($user_images_id){		$s = $this->db->select("avg(*) as avg")			->where("user_images_id", $user_images_id)->get("user_images_rating");		$r = $s->row();		if(isset($r->num)) return $r->num;		return 0;	}
	...
	{			$q=$this->db->where("userid", $userid)->get("user_albums");
    ...
    {			$iq=$this->db->where("user_images.albumid", $id)			/*->select("user_images.ID, user_images.name, user_images_rating.user_images_id,avg(user_images_rating.rating) as avg")->join("user_images_rating", "user_images_rating.user_images_id = user_images.ID", "left outer")*/    		->limit(50, $page)->get("user_images");
    	return $iq->result_array();
    }
 ...
 public function add_image_rating($data)     {    	$this->db->insert("user_images_rating", $data);    	return $this->db->insert_ID();    }
}
