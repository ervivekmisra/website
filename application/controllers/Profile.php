...
class Profile extends CI_Controller 
{
	...
	public function index($username="") 
	{
		...
		if( $albums->num_rows() > 0){
            $album = $albums->row();
            $images = $this->image_model->get_album_images($album->ID, 0);
        }
	...
	public function albums($userid) 
	{
        ...
        $album = $this->image_model->get_user_albums_all($userid);
        if($album->num_rows() == 0){   //Default Album Add
        ...
        $config['base_url'] = site_url("profile/albums/" . $userid);
        $config['total_rows'] = $this->image_model->get_total_album_images($album->ID);
        $config['per_page'] = 50;
...
//
//		if($albums->num_rows() == 0){   //Default Album Add
//            $this->image_model->add_album(array(
//                    "userid" => $user->ID,
//                    "name" => "",
//                    "description" => "",
...
//                if($user->ID == $this->user->info->ID || $this->common->has_permissions(array("admin","admin_members"), $this->user)) {
...
//            }
//            echo
//                json_encode($this->datatables->process());
//        }
//
//	}
//
...
//		$this->session->set_flashdata("globalmsg", lang("success_71"));
		redirect(site_url("profile/albums/" . $userid));
	}
	...
	public function edit_image($id) 
	{
		...
		$albums = $this->image_model->get_user_albums_all($this->user->info->ID);
	...
	public function edit_image_pro($id) 
	{
		...
		redirect(site_url("profile/albums/" . $image->userid));
	...
	public function add_friend($userid) 
	{
		...
		if($request->num_rows() > 0) {
			$this->template->jsonError(lang("error_143"));
		}
		...
		// Notification
		$this->user_model->increment_field($user->ID, "noti_count_friend", 1);
