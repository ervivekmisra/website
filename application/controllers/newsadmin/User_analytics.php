...
class User_analytics extends CI_Controller{	
	...
	public function index(){
		...
		$f = date("Y-m-d h:i:s", strtotime("-30 second "));
		$t = date("Y-m-d h:i:s");
...
public function location_based(){
        ...
        @$country_name = $this->input->get('country');
        ...
        // pagination settings
        $total_rows = $rows;
        $config["base_url"] = base_url("newsadmin/User_analytics/location_based/");
...
public function news_based(){
	...
	public function clear_analytics(){
		$data['status'] = $this->vsm->get_previous_settings('settings', 8);
	...
	public function analytics_status($status){
		...
		redirect('newsadmin/User_analytics/clear_analytics');
	}
