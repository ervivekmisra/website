...
#------------------------------------
    ...
    if($_FILES['cate_pic']['name']){
        ...
        if (!$this->upload->do_upload('cate_pic')) {
            ...
            redirect('newsadmin/Category/add_category');
        }
        else
        {
    ...
    if($_FILES['cate_pic']['name']){    
        ...
        if (!$this->upload->do_upload('cate_pic')) {
            ...
            $this->session->set_flashdata('exception',$error);
             redirect('newsadmin/Category/add_sub_categories');
        }
        else
        {
         $data1 = $this->upload->data();
...
#-----------------------------------------------    
    ...
    if ($_FILES['cate_pic']['name']) {
        ...
        if (!$this->upload->do_upload('cate_pic')) {
            $error =  $this->upload->display_errors();
            $this->session->set_flashdata('exception',$error);
            redirect('newsadmin/Category/update_view/' . $category_id);
        }
        else
        {
         $data1 = $this->upload->data();
       ...
       $data['category_imgae'] = $this->input->post('pic'); 
        ...
        redirect('newsadmin/Category/list_of_categories');
    }
