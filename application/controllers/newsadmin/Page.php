...
public function Save_page() {
    ...
    if (@$_FILES['image']['name']){
      ...
      if (!$this->upload->do_upload('image')){
          ...
          redirect("newsadmin/Page/Create_new_page");
      } else {
    ...
    $this->db->insert('news_pages',$data);
...
$id = $this->input->post('id');
    ...
    if (@$_FILES['image']['name']!=NULL){
      ...
      if (!$this->upload->do_upload('image')){
          $this->session->set_flashdata('exception','This File Not allowed!');
          redirect("newsadmin/Page/Edit_page/".$id);
      } else {
        $data = $this->upload->data();
        $image = $config['upload_path'].$data['file_name'];
      }
...
// print_r($data); exit;
    ...
    $this->db->where('page_id',$id)->update('news_pages',$data);

    $this->session->set_flashdata('message', display('update_message'));
    redirect("newsadmin/Page/Pages");
}

public function Delete_page($id){
    $this->db->where('page_id',$id)->delete('news_pages');
