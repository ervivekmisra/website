...
//		if($submit_type == 1)
		{
            ...
            $birthday = $this->common->nohtml($this->input->post("birthday"));
            ...
            if($this->settings->info->avatar_upload) {
                ...
                if ($_FILES['userfile']['size'] > 0) {
                    ...
                    $image= $this->user->info->avatar_source;
                }


                $base64_string = $this->common->nohtml($this->input->post("filename_crop"));;
                if($base64_string != ""){
                    /* Extract the filename */
                    $image_crop = $this->user->info->ID."_avatar.png";//.substr($filename, strrpos($filename, '/') + 1);
                    /* Save file wherever you want */
                    $base64_string = explode( ',', $base64_string );
                    $filename_content = base64_decode($base64_string[1]);
                    file_put_contents($this->settings->info->upload_path."/". $image_crop, $filename_content);
                ...
                if ($_FILES['userfile_profile']['size'] > 0) {
                    ...
                    $profile_header = $this->user->info->profile_header_source;
                }



                $base64_string = $this->common->nohtml($this->input->post("profile_header_crop"));;
                if($base64_string != ""){
                    /* Extract the filename */
                    $image_crop_header = $this->user->info->ID."_profile_header.png";//.substr($filename, strrpos($filename, '/') + 1);
                    /* Save file wherever you want */
                    $base64_string = explode( ',', $base64_string );
                    $filename_content = base64_decode($base64_string[1]);
                    file_put_contents($this->settings->info->upload_path."/". $image_crop_header, $filename_content);
...
//                    "profile_header" => $profile_header,
                    ...
                    "profile_header" => $image_crop_header,
                    "profile_header_source" => $profile_header,
                    "avatar" => $image_crop,
                    "avatar_source" => $image
                )
            );
	...
	public function friend_request($type, $id, $hash) 
	{
        ...
        $this->user_model->decrement_field($this->user->info->ID, "noti_count_friend", 1);
