	...
	public function __construct() 
	{
		...
		$this->oauth_secret = $CI->input->cookie($config . "oauthsecret", TRUE);
 		...
 		$select = "users.`ID`, users.`username`, users.`email`, 
				...
				users.location_live, users.friends, users.pages, users.tag_user,users.avatar_source, users.profile_header_source, users.noti_count_friend, users.birthday,
...
//                    ->get("user_friend_requests");
				...
				$friend_reqs = $CI->db->select("id")->where("friendid", $this->info->ID)->get("user_friend_requests");
				if($friend_reqs != null)
					$this->info->noti_count_friend = $friend_reqs->num_rows();
