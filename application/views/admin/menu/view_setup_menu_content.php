                  ...
                  <div class="tab-content">
                    ...
                    <a style="margin-top: 10px;"  class="btn btn-success" href="<?php echo base_url();?>add_category"><i class="glyphicon glyphicon-plus"></i> <?php echo display('add_category')?></a>
                        <?php 
                            ...
                            echo form_open_multipart('newsadmin/Menu/Save_content_menu', $attributes);
                        ?>
                    ...
                    <div id="Pages" class="tab-pane fade">
                        <a style="margin-top: -20px" class="btn btn-success" href="<?php echo base_url()?>newsadmin/Page/Create_new_page"><i class="glyphicon glyphicon-plus"></i> <?php echo display('add_page')?></a>
                        <?php 
                            $attributes = array( 'class' => 'form-horizontal','method'=>'post');
                            echo form_open_multipart('newsadmin/Menu/Save_page_content_menu', $attributes);
                        ?>
                      <h3>Pages</h3>
                    ...
                    <div id="Link" class="tab-pane fade">
                        <button style="margin-top: -20px"  class="btn btn-success" onclick="add_link(<?php echo $menu_id?>)"><i class="glyphicon glyphicon-plus"></i> <?php echo display('addnewlink')?></button>
                            <?php 
                                $attributes = array( 'class' => 'form-horizontal','method'=>'post');
                                echo form_open_multipart('newsadmin/Menu/Add_link_with_content', $attributes);
                            ?>
...
function ContentPositionUpdate(){
    var url = "<?php echo site_url('newsadmin/Menu/DragDrop_update')?>";
...
function set_status($id){
     var url = "<?php echo site_url('newsadmin/Menu/Udate_status')?>/"+id;
...
function edit_menu(id)
{
    ...
    $('.form-group').removeClass('has-error'); // clear error class
    $('.help-block').empty(); // clear error string
    var url = "<?php echo site_url('newsadmin/Menu/Content_menu_data')?>/"+id;
...
function save()
{
    ...
    var url;
    if(save_method == 'add') {
        url = "<?php echo site_url('newsadmin/Menu/Save_link')?>";
    } else {
        url = "<?php echo site_url('newsadmin/Menu/Content_menu_update')?>";
    }
    // ajax adding data to database
    $.ajax({
        url : url,
...
function delete_menu(id)
{
    ...
    {
         var url = "<?php echo site_url('newsadmin/Menu/Delete_content_menu')?>/"+id;
        // ajax delete data to database
        $.ajax({
            url : url,
            type: "GET",
