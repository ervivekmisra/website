...
<script src="<?php echo base_url();?>croppie/croppie.js"></script>
<link href="<?php echo base_url();?>croppie/croppie.css" rel="stylesheet" media="screen">
...
<div class="db-header clearfix">
    ...
    <div class="page-header-title page_head"> <span class="glyphicon glyphicon-cog"></span> <?php echo lang("ctn_224") ?></div>
...
<?php echo form_open_multipart(site_url("user_settings/pro"), array("class" => "form-horizontal")) ?>
	...
	<div class="form-group">
	    ...
	    <div class="col-sm-10">
            ...
            <div class="div_upload_avata" id="div_upload_avata" width="200px" height="200px"></div>

	    <?php if($this->settings->info->avatar_upload) : ?>

	     	<input class="fileUpload" type="file" name="userfile" />
            <button class="save_btn btn btn-success btn-sm">Save</button>
	...
	<div class="form-group">
	    ...
	    <div class="col-sm-10">
            ...
            <div class="div_upload_profile_header" id="div_upload_profile_header" height="500px"></div>

	    <?php if($this->settings->info->avatar_upload) : ?>

	     	<input class="fileUpload" type="file" name="userfile_profile" />
            <button class="save_btn btn btn-success btn-sm">Save</button>
    ...
    <div class="form-group">
        ...
        <label for="inputEmail3" class="col-sm-2 control-label"><?php echo lang("ctn_756") ?></label>

        <div class="col-sm-10">

            <input type="date" class="form-control" name="birthday" value="<?php echo $this->user->info->birthday ?>">
    ...
    <input type="hidden" name="filename_crop" id="filename_crop" />
    <input type="hidden" name="profile_header_crop" id="profile_header_crop" />

	 <input type="button" class="save_btn" value="<?php echo lang("ctn_236") ?>" class="btn btn-primary form-control" />
...
$(document).ready(function() {
    ...
    var upload_avataCrop, upload_bannerCrop;

    function readFile(input, upload_crop, divname) {
        if (input.files && input.files[0]) {
            var reader = new FileReader();

            reader.onload = function (e) {
                $(divname).addClass('ready');
    ...
    upload_avataCrop = $('#div_upload_avata').croppie({
        viewport: {
            width: 150,
            height: 150
        },
    ...
    var userfile_url = '<?php echo $this->user->info->avatar_source ?>';
    if( userfile_url == '')
        userfile_url = '<?php echo $this->user->info->avatar ?>';
    if (userfile_url != '')
    {
        $('.div_upload_avata').addClass('ready');
        $('#div_upload_avata').croppie('bind', {
            url: '<?php echo base_url().'/'.$this->settings->info->upload_path_relative.'/'?>'+userfile_url,
        });
    }


    $('input[name=userfile').on('change', function () { readFile(this, upload_avataCrop, '#div_upload_avata'); });


    boundary_width = $('#div_upload_profile_header').width();
    view_width = boundary_width * 0.8
    boundary_height = boundary_width * 30 / 111;

    view_height = view_width * 30 / 111;


    upload_profileHeaderCrop = $('#div_upload_profile_header').croppie({
        viewport: {
            width: view_width,
            height: view_height
        },
    ...
    var profile_header_url = '<?php echo $this->user->info->profile_header_source ?>';
    if( profile_header_url == '')
        profile_header_url = '<?php echo $this->user->info->profile_header ?>';

    if (profile_header_url != '')
    {
        $('.div_upload_profile_header').addClass('ready');
        $('#div_upload_profile_header').croppie('bind', {
            url: '<?php echo base_url().'/'.$this->settings->info->upload_path_relative.'/'?>'+profile_header_url,
        });
    }


    $('input[name=userfile_profile').on('change', function () { readFile(this, upload_profileHeaderCrop, '#div_upload_profile_header'); });
    ...
    function crop_image(){
        upload_profileHeaderCrop.croppie('result', {
            type: 'canvas',
            ...
            $('#profile_header_crop').val(resp);

            upload_avataCrop.croppie('result', {
                type: 'canvas',
