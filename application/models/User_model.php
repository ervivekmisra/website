...
class User_Model extends CI_Model 
{
	...
	public function get_user_members() 
	{
		return $this->db->select("users.username, users.email, users.first_name, 
			users.last_name, users.ID, users.joined, users.oauth_provider,
			users.user_role, users.online_timestamp, users.avatar,allow_friends,
			user_roles.name as user_role_name")
		->join("user_roles", "user_roles.ID = users.user_role", 
				 	"left outer")
		->order_by('FIELD(users.avatar, "default.png")')
		->get("users");
	}
    ...
    public function get_members_by_birthday($search)
    {
        return $this->db->select("users.username, users.email, users.first_name, users.email, users.email_notification,
			users.last_name, users.ID, users.birthday")
            ->like("users.birthday", $search)
            ->get("users");
    }
    ...
    public function update_field($userid, $field, $value)
    {
        $this->db->where("ID", $userid)
            ->set($field, $value, FALSE)->update("users");
    }
    ...
    {
        return $this->db->where("ID>=", 7)->get("user_roles");
    }
    ...
    public function get_friends_ids($userid)
    {
        return $this->db
            ->where("userid", $userid)
            ->select("friendid")
    ...
    public function check_friend($userid, $friendid) 
	{
		...
		$friend_flag = 0;
		$request_flag = 0;
		$friend = $this->get_user_friend($userid, $friendid);
		if($friend->num_rows() > 0) {
			// Friends
			$friend_flag = 1;
		} else {
			// Check for a request
			$request = $this->check_friend_request($userid, $friendid);
